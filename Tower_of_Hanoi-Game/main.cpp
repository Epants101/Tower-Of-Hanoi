...
const unsigned int numberOfRings = 5;
...
int main(void)
{
    ...
    while (!WindowShouldClose() || errorHasOccured)    // Detect window close button or ESC key
    {
        ...
        //Check for "s" for screenshot
        if (IsKeyPressed(83)) 
        {
            screenshotFileName = "Disks" + std::to_string(numberOfRings) + "Move" + std::to_string(moveNumber) + ".png";
            TakeScreenshot(screenshotFileName.c_str());
        }
        ...
        //Draw the disks
        if (!drawDisks(rodWidth, rodHeight - baseDepth, rodDiameter, rodY, rodXs, disks, baseDepth/2, selectedDisk)) {
            errorHasOccured = true;
        }
